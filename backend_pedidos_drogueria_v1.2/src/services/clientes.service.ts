...
export class ClientesServices {
    ...
    static async getRiesgo(codcliente: number): Promise<any> {
        try {
            ...
            const result = await pool.request()
                ...
                .input('VED_CODE', vedCode)
                .query(`SELECT
                            CL.CODCLIENTE
                            , CL.NOMBRECLIENTE
                            , CL.RIESGOCONCEDIDO
                            , ISNULL(SUM(CASE WHEN ISNULL(T.CODMONEDA,1) = @USD_CODE THEN T.IMPORTE ELSE T.IMPORTE / NULLIF(DBO.F_GET_COTIZACION(GETDATE(), @VED_CODE), 0) END), 0) CX
                            , CASE WHEN CL.RIESGOCONCEDIDO <> 0 THEN (ISNULL(SUM(CASE WHEN ISNULL(T.CODMONEDA,1) = @USD_CODE THEN T.IMPORTE ELSE T.IMPORTE / NULLIF(DBO.F_GET_COTIZACION(GETDATE(), @VED_CODE), 0) END), 0) * 100) / CL.RIESGOCONCEDIDO ELSE 0 END PORCENTAJE_RIESGO
                            , CASE
                                WHEN (CASE WHEN CL.RIESGOCONCEDIDO <> 0 THEN (ISNULL(SUM(CASE WHEN ISNULL(T.CODMONEDA,1) = @USD_CODE THEN T.IMPORTE ELSE T.IMPORTE / NULLIF(DBO.F_GET_COTIZACION(GETDATE(), @VED_CODE), 0) END), 0) * 100) / CL.RIESGOCONCEDIDO ELSE 0 END) BETWEEN 80 AND 99 THEN 'ALTO'
                                WHEN (CASE WHEN CL.RIESGOCONCEDIDO <> 0 THEN (ISNULL(SUM(CASE WHEN ISNULL(T.CODMONEDA,1) = @USD_CODE THEN T.IMPORTE ELSE T.IMPORTE / NULLIF(DBO.F_GET_COTIZACION(GETDATE(), @VED_CODE), 0) END), 0) * 100) / CL.RIESGOCONCEDIDO ELSE 0 END) >= 100 THEN 'SUPERADO'
                                WHEN (CASE WHEN CL.RIESGOCONCEDIDO <> 0 THEN (ISNULL(SUM(CASE WHEN ISNULL(T.CODMONEDA,1) = @USD_CODE THEN T.IMPORTE ELSE T.IMPORTE / NULLIF(DBO.F_GET_COTIZACION(GETDATE(), @VED_CODE), 0) END), 0) * 100) / CL.RIESGOCONCEDIDO ELSE 0 END) BETWEEN 30 AND 79 THEN 'MEDIO'
                                ELSE 'BAJO'
                              END ESTATUS
                        FROM
                            CLIENTES CL
    ...
    static async getRiesgoMasivo(codclientes: number[]): Promise<any> {
        try {
            ...
            request.input('VED_CODE', vedCode);

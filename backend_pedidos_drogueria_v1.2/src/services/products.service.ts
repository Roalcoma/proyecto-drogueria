...
export const STOCK_DISPONIBLE_SQL = `(
    ...
    - ISNULL((
        ...
        WHERE LP.CODARTICULO = A.CODARTICULO
            AND CP.ESTATUS IN ('PENDIENTE POR AUTORIZACION', 'APROBACION PSICOTROPICOS', 'SANIDAD', 'AUTORIZADO', 'EMPACADO', 'OK')
    ), 0)
)`;
            ...
            .query(`
                SELECT DISTINCT A.CODARTICULO, A.REFPROVEEDOR,
                    ...
                    (SELECT TOP 1 COALESCE(TRY_CONVERT(DATE, AL.GARANTIACOMPRA, 103), TRY_CONVERT(DATE, AL.GARANTIACOMPRA, 23))
                     FROM ARTICULOSLIN AL WITH(NOLOCK) WHERE AL.CODARTICULO = A.CODARTICULO
                     ORDER BY COALESCE(TRY_CONVERT(DATE, AL.GARANTIACOMPRA, 103), TRY_CONVERT(DATE, AL.GARANTIACOMPRA, 23)) DESC) AS GARANTIACOMPRA,

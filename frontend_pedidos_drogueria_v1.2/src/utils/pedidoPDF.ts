...
async function renderPedidoEnDoc(doc: jsPDF, data: PedidoPDFData, preloadedLogo?: string | null): Promise<void> {
    ...
    doc.text(
        maxDiasProteccion > 0
            ? (isPsico ? 'Monto Factura' : 'Monto Factura — NO INDEXADO')
            : 'Indexada',
        30, datosFin - 1
    );
    ...
    // --- Tabla de líneas ---
    // Helper: formatea un descuento individual como "X%" o "" si es 0/nulo
    const fmtD = (val?: number) => (val && val > 0) ? `${val}%` : '';
    ...
    const filas: any[][] = isPsico
        ? data.lineas.map(l => {
            ...
            const d = l.sinDescuento || ocultarDescuentos ? [] : (l.descuentos ?? []);
            const pct = l.porcentajeIva ?? 0;
            return [l.codigo, (l.descripcion || '') + (l.esControlado ? ' (CONTROLADO)' : ''),
                l.cantidad,
                sinDescCab ? '' : fmtD(d[0]),  // ESC PRD = D1
                fmtD(d[1]),                     // ESC PRD = D2
                fmtD(d[2]),                     // ESC PRV = D3
                fmtD(d[3]),                     // DESC    = D4
                pct > 0 ? `+${pct}%` : '',
